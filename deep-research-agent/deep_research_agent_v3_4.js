/* METADATA
{
  "name": "deepsight_v4_5_1",
  "display_name": {
    "zh": "深度搜索工具 v4.5.1",
    "en": "DeepResearch Tools v4.5.1"
  },
  "description": {
...
var ERROR = {
 ...
 (function() {
    ...
    // ── Metrics stub (v4.5.1) — referenced by advanceSearchRound ──
    if (typeof Metrics === 'undefined') {
        var Metrics = { _m: {}, inc: function(k) { this._m[k] = (this._m[k] || 0) + 1; }, snapshot: function() { return JSON.parse(JSON.stringify(this._m)); } };
    }
    ...
    var _activeSessionCache = null; // v4.5.2: BUG-6 — cached session state for status queries
    ...
    function _updateMenuState(stage) {
        ...
        try {
            ...
            if (typeof console !== 'undefined') console.error('[DeepResearch] _updateMenuState error: ' + String(e));
        }
    }
 ...
 // ── _classifyError: map JS exceptions to ERROR codes (v3.9.0 Phase2) ──
     ...
     function _writeAutoCheckpoint(stage, ss) {
         ...
         try {
             ...
             if (files && files.length > 50) {
              ...
              if (typeof console !== 'undefined') console.error('[DeepResearch] _writeAutoCheckpoint error: ' + String(e));
          }
     }
     ...
     function _wrapTool(spec) {
         return async function(params) {
             ...
             // 7. Update menu state + pipeline stage tracking
            if (spec.stage) {
                _updateMenuState(spec.stage);
                // v4.5.2: P2-2 — sync currentStage and completedStages on every tool call
                if (ss) {
                    if (ss.currentStage !== spec.stage) {
                        if (!ss.completedStages) ss.completedStages = [];
                        if (ss.completedStages.indexOf(spec.stage) === -1) {
                            ss.completedStages.push(spec.stage);
                        }
                        ss.currentStage = spec.stage;
                    }
                }
            }
            ...
            // v4.5.2: BUG-6 — cache active session for status queries without session_state param
            if (ss && result && result.success !== false) {
                _activeSessionCache = ss;
            }
            return result;
         };
     }
    ...
    var _advRoundState = {};  // sid_round → true  Map (v4.5.2: composite key to avoid sessionId collision)
    ...
    function advanceSearchRound(sessionState) {
        ...
        var compositeKey = sid + '_' + currentRound;  // v4.5.2: BUG-2 fix — use sid+round composite key
        if (_advRoundState.hasOwnProperty(compositeKey)) return;
        _advRoundState[compositeKey] = true;
...
// ── Authority Policy (v3.3.0→v3.4.0) ─────────────────────────────
    ...
    var AuthorityPolicy = (function() {
        ...
        function register(language, matcherFn) {
            ...
            var _langMatchers = {};
    // v4.5.2: D5 fix — unified inline domain database, no external file dependency
    // Add new domains here; entries are ordered by tier priority within each language group.
    var AUTHORITY_DOMAIN_DB = {
        zh: [
            // Tier 0 — National Authority
            { tier: 0, level: 'tier0', label: 'National Authority', score: 1.0, authority: 'government', hosts: ['caict.ac.cn','cncert.org.cn','cac.gov.cn','miit.gov.cn','stats.gov.cn','moe.gov.cn'] },
            // Tier 1 — Top Academic
            { tier: 1, level: 'tier1', label: 'Top Academic', score: 0.9, authority: 'academic', hosts: ['cnki.net','wanfangdata.com','cas.cn','cae.cn','ccf.org.cn','nsfc.gov.cn'] },
            // Tier 2 — Official Tech Blog
            { tier: 2, level: 'tier2', label: 'Official Tech Blog', score: 0.7, authority: 'industry-official', hosts: ['tech.meituan.com','developer.huawei.com','tech.bytedance.com'] },
            // Tier 2 — Chinese Financial Media
            { tier: 2, level: 'tier2', label: 'Chinese Financial Media', score: 0.65, authority: 'industry-report', hosts: ['eastmoney.com','stcn.com','xueqiu.com','jiemian.com','cls.cn'] },
            // Tier 3 — Verified Community (author+verified required for tier 3, else UGC)
            { tier: 3, level: 'tier3', label: 'Verified Community', score: 0.45, authority: 'community-verified', hosts: ['csdn.net','juejin.cn','segmentfault.com','zhihu.com','cnblogs.com'], requiresVerified: true },
            // Tier 3 — Tech Edu
    { tier: 3, level: 'tier3', label: 'Tech Edu', score: 0.4, authority: 'tech-edu', hosts: ['blog.51cto.com'] },
    // Tier 3 — Verified Medical Content (doctor-reviewed health info)
    { tier: 3, level: 'tier3', label: 'Verified Medical Content', score: 0.5, authority: 'medical', hosts: ['health.baidu.com'] },
            // Tier 4 — UGC (default)
            { tier: 4, level: 'tier4', label: 'UGC', score: 0.15, authority: 'ugc', role: 'clue_source', hosts: [] }
        ],
        en: [
            // Tier 0 — Primary Source
            { tier: 0, level: 'tier0', label: 'Primary Source', score: 0.95, authority: 'primary', hosts: ['github.blog','anthropic.com','openai.com','arxiv.org','nist.gov','owasp.org','acm.org','ieee.org','stackoverflow.com'] },
            // Tier 1 — Academic/Think Tank
            { tier: 1, level: 'tier1', label: 'Academic/Think Tank', score: 0.85, authority: 'academic', hosts: ['nature.com','science.org','springer.com','elsevier.com','cambridge.org','mit.edu','stanford.edu','berkeley.edu','cmu.edu','ox.ac.uk','cam.ac.uk','rand.org','brookings.edu','csis.org'] },
            // Tier 2 — Industry/Tech Media
            { tier: 2, level: 'tier2', label: 'Industry/Tech Media', score: 0.6, authority: 'industry-report', hosts: ['gartner.com','forrester.com','idc.com','techcrunch.com','wired.com','theverge.com','arstechnica.com'] },
            // Tier 3 — Expert Blog
            { tier: 3, level: 'tier3', label: 'Expert Blog', score: 0.3, authority: 'expert-blog', hosts: ['medium.com','dev.to','substack.com'] },
            // Tier 4 — Personal/UGC
            { tier: 4, level: 'tier4', label: 'Personal/UGC', score: 0.1, authority: 'ugc', hosts: ['blogspot','wordpress','github.io','netlify.app','vercel.app'] }
        ]
    };
    // v4.5.2: Unified domain lookup — iterate DB entries in order, first match wins.
    function matchFromDB(host, url, lang, metadata) {
        var entries = AUTHORITY_DOMAIN_DB[lang] || AUTHORITY_DOMAIN_DB.en;
        for (var ei = 0; ei < entries.length; ei++) {
            var entry = entries[ei];
            for (var hi = 0; hi < entry.hosts.length; hi++) {
                if (host.indexOf(entry.hosts[hi]) !== -1 || url.indexOf(entry.hosts[hi]) !== -1) {
                    if (entry.requiresVerified) {
                        if (metadata && metadata.author && metadata.verified) {
                            return { tier: entry.tier, level: entry.level, label: entry.label, score: entry.score, authority: entry.authority };
                        }
                        return { tier: 4, level: 'tier4', label: 'UGC', score: 0.15, authority: 'ugc', role: 'clue_source' };
                    }
                    return { tier: entry.tier, level: entry.level, label: entry.label, score: entry.score, authority: entry.authority };
                }
            }
    ...
    // v4.5.2: Heuristic fallback when no DB entry matches
    function matchHeuristic(host, url) {
        if (host.indexOf('.edu') !== -1) return { tier: 2, level: 'tier2', label: 'Educational', score: 0.5, authority: 'academic' };
        if (host.indexOf('.gov') !== -1) return { tier: 2, level: 'tier2', label: 'Government', score: 0.6, authority: 'government' };
        if (host.indexOf('baike.baidu.com') !== -1) return { tier: 2, level: 'tier2', label: 'Encyclopedia', score: 0.5, authority: 'reference' };
        if (host.indexOf('wikipedia.org') !== -1) return { tier: 2, level: 'tier2', label: 'Encyclopedia', score: 0.5, authority: 'reference' };
        return null;
    }
    _langMatchers.zh = function(host, url, md) {
        var r = matchFromDB(host, url, 'zh', md);
        if (!r) r = matchFromDB(host, url, 'en', md);
        if (!r) r = matchHeuristic(host, url);
        return r;
    };
    _langMatchers.en = function(host, url, md) {
        var r = matchFromDB(host, url, 'en', md);
        if (!r) r = matchFromDB(host, url, 'zh', md);
        if (!r) r = matchHeuristic(host, url);
        return r;
    };
    // v4.5.2: migrate from legacy hardcoded functions — keep as aliases for backward compat
    function matchCnTier(host, url, metadata) { return _langMatchers.zh(host, url, metadata || {}); }
    function matchEnTier(host, url, metadata) { return _langMatchers.en(host, url, metadata || {}); }
...
// ── Source Index (v3.3.0) ──────────────────────────────────
    ...
    function classifyAuthorityV4(url, metadata) {
        ...
        if (result.tier === 4) {
            ...
            else if (host.indexOf('baike.baidu.com') !== -1 || host.indexOf('wikipedia.org') !== -1) {
    ...
    function matchEnTier(host, url, metadata) {
        ...
        var t1 = ['nature.com','science.org','springer.com','elsevier.com','cambridge.org','mit.edu','stanford.edu','berkeley.edu','cmu.edu','ox.ac.uk','cam.ac.uk','cset.georgetown.edu','rand.org','brookings.edu','csis.org','cfr.org','belfercenter.org','cnas.org','iiss.org','chathamhouse.org','wilsoncenter.org','carnegieendowment.org'];
...
name: 'create_checkpoint',
    ...
    var sync_to_plan = _wrapTool({
        ...
        impl: async function(params, ss) {
            ...
            if (action === 'complete' && typeof EventBus !== 'undefined') { EventBus.emit('pipeline:idle', { ts: new Date().toISOString() }); }
            if (action === 'complete') { _updateMenuState('OUTPUT'); }
...
// v4.5.2: Two-mode dispatch — FORCE executes + closes toggle (finally),
// OFF prompts to enable. SUGGEST mode removed.
...
var sessionState = base.sessionState;
    ...
    // v4.5.2: BUG-4 — emit pipeline:start for main.js EventBus subscriber
    if (typeof EventBus !== 'undefined') {
        EventBus.emit('pipeline:start', { sessionId: sessionState.sessionId, query: query, stage: 'QUERY_PLAN' });
    }
    ...
    // v4.5.2: P0-3 — cache session for status queries
    _activeSessionCache = sessionState;
...
// ── Record failure result ──
        ...
        if (isForce) {
            try {
                ...
                // AWAIT_INPUT_KEY preserved across orchestration window; cleared on pipeline:idle
            } catch (e) {
...
if (autoAdvance) {
  ...
  var advance_search_round = _wrapTool({
        ...
        impl: async function(params, ss) {
            var sessionState = ss;
            // v4.5.1: guard — reject if rounds>0 but no sources ingested
            var rds = (sessionState.search && sessionState.search.rounds) || 0;
            var ing = (sessionState.search && sessionState.search.totalIngested) || 0;
            if (rds > 0 && ing === 0) {
                return {
                    success: false,
                    _required_action: 'ingest_source',
                    required_next_step: { step_id: 'step_' + rds, step_type: 'ingest_source', required_fields: ['url'], deadline_ms: 10000, failure_consequences: '若未完成 ingestion，将中止任务', evidence_required: true },
                    refusal_reason: '第' + rds + '轮搜索后未执行任何 ingest_source',
                    punishment: true,
                    error: 'BLOCKED: must call ingest_source at least once before advancing search round.'
                };
            }
            advanceSearchRound(sessionState);
            var rounds = (sessionState.search && sessionState.search.rounds) || 0;
...
async function check_research_status(params) {
    try {
        var ss = (params && params.session_state) || _activeSessionCache || (typeof sessionState !== 'undefined' ? sessionState : null);
        var body = {
            toggle: 'off',
            pipeline: 'idle',
            stage: null,
...
// v4.5.1: orchestrate_research call tracker
(function() {
    var _lastOrchTs = null;
    var _origOrch = orchestrate_research;
    if (typeof _origOrch === 'function') {
        orchestrate_research = function() {
            _lastOrchTs = Date.now();
            return _origOrch.apply(this, arguments);
        };
        if (typeof EventBus !== 'undefined' && typeof EventBus.emit === 'function') {
            setTimeout(function() {
                if (_lastOrchTs === null) {
                    EventBus.emit('deepsight_orchestrate_timeout', { message: '警告：已激活工具包但尚未调用orchestrate_research。' });
                }
            }, 30000);
        }
    }
...
// v4.5.1: self-check — 10s timeout if orchestrate_research not called
(function() {
    var _orchCalled = false;
    var _origOrch = orchestrate_research;
    if (typeof _origOrch === 'function') {
        orchestrate_research = function() {
            _orchCalled = true;
        ...
        setTimeout(function() {
            if (!_orchCalled) {
                var msg = 'WARNING: deepsight_v450_tools active but orchestrate_research not called within 10s';
                if (typeof EventBus !== 'undefined' && typeof EventBus.emit === 'function') {
                    EventBus.emit('deepsight_warning', { message: msg });
                }
